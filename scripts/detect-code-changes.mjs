 ...
 *   - rs-changed: 'true' if any .rs files changed (rust/ folder)
 *   - toml-changed: 'true' if any .toml files changed
 *   - js-changed: 'true' if any .js files changed (js/ folder)
...
function detectChanges() {
  ...
  // Detect .toml file changes (Cargo.toml in rust/ folder)
  ...
  // Detect .mjs file changes (scripts in any folder)
  ...
  const rustCodePattern = /^rust\/.*\.(rs|toml)$/;
  const anyRustCodeChanged = codeChangedFiles.some((file) =>
    rustCodePattern.test(file)
  );
  setOutput('any-rust-code-changed', anyRustCodeChanged ? 'true' : 'false');

  // Check if any JS code files changed
  const jsCodePattern = /^js\/.*\.(js|cjs|mjs|json)$/;
  const anyJsCodeChanged = codeChangedFiles.some((file) =>
    jsCodePattern.test(file)
  );

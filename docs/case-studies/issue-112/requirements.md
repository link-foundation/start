...
| R2  | `--stop` should send CTRL+C or equivalent graceful stop request to the command process. | screen uses `screen -S <session> -X stuff <Ctrl-C>`, tmux uses `send-keys C-c`, Docker uses `docker stop`.            |

...
use start_command::{
    ...
    query_commands::dispatch_query_command,
...
fn main() {
    ...
    let query_store = config.create_execution_store();
    if let Some(exit_code) =
        dispatch_query_command(&wrapper_options, query_store.as_ref(), &parsed_command)
    {

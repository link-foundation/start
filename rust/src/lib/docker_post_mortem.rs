...
/// fallback strips them before parsing and adds them back afterwards.
/// Truncating both timestamps to whole seconds instead would *not* cancel out: a
/// container alive from `.942` to `.740` of the next-but-five second measures
/// 5.798s but truncates to 6.000s, and the error can reach a full second either
/// way.
